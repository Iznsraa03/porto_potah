...
type TextTypeProps<T extends ElementType = 'div'> = {
  ...
  /** Solid color per sentence (overrides CSS color). */
  textColors?: string[];
  /** CSS class(es) per sentence — applied to the typed text span. */
  textClasses?: string[];
...
const TextType = <T extends ElementType = 'div'>({
  text,
  ...
  const currentClass = getCurrentTextClass();
  ...
  return createElement(
    ...
    // Zero-width space (\u200b) prevents height collapse when displayedText is ''
    <span
      className={`text-type__content ${currentClass}`}
